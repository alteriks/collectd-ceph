...
class CephPoolPlugin(base.Base):
    ...
    def get_stats(self):
        ...
        stats_output = None
        try:
            osd_pool_cmdline='docker exec ceph_mon ceph osd pool stats -f json --cluster ' + self.cluster
            stats_output = subprocess.check_output(osd_pool_cmdline, shell=True)
            cephdf_cmdline='docker exec ceph_mon ceph df -f json --cluster ' + self.cluster 
